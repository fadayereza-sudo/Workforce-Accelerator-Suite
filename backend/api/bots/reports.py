"""
Reports API - Activity monitoring and LLM-generated report endpoints.
...
async def list_reports(
    org_id: str,
    report_type: Optional[str] = Query(None),  # 'team', 'agent'
    period_type: Optional[str] = Query(None),  # 'daily', 'weekly', 'monthly'
...
async def get_latest_reports(
    ...
    # Get latest agent reports (one per bot)
    ...
    # Dedupe to get only the latest per bot
    seen_bots = set()
    agent_reports = []
